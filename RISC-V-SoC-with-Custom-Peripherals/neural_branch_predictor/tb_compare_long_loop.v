...
// Long strongly-taken loop test: x1=32, countdown to 0.
// bne x1, x0, -12 at PC=28: taken 31 times, not-taken 1 time (32 total).
// Baseline (always-NT): 1/32 = 3.125% accuracy, 31 mispredictions, 62 penalty cycles.
// Innovation: bimodal reaches "strongly taken" state within 2-3 iterations,
//             then handles all remaining via high-confidence bimodal path.
// Expected at end: x1=0, x30=77.

module tb_compare_long_loop;
    ...
    initial begin
        ...
        $display("\n==============================================================================");
        $display(" Long Strongly-Taken Loop: Baseline CPU vs Innovation CPU");
        $display(" Program: x1=32 countdown, bne x1,x0,-12 (31T + 1NT = 32 branches)");
        $display(" Baseline: 1/32=3.125%%, 31 mispred, 62 penalty");
        $display(" Expected: x1=0, x30=77");
        $display("==============================================================================");
    ...
    initial begin
        $readmemb("/home/chiawei/SiliconAwards/RISC-V-SoC-with-Custom-Peripherals/neural_branch_predictor/instr_data_long_loop.txt",
    ...
    initial begin
        ...
        $display("------------------------------------------------------------------------------");
        $display("LONG STRONGLY-TAKEN LOOP -- FINAL COMPARISON");
        $display("------------------------------------------------------------------------------");
        ...
        $display("  x1  final = %0d (expected 0)",  bp_x1_final);
        $display("  x30 final = %0d (expected 77)", bp_x30_final);
        if ((bp_x1_final == 32'd0) && (bp_x30_final == 32'd77))
            $display("  STATUS: PASS");
        else
            $display("  STATUS: FAIL");
        $display("------------------------------------------------------------------------------");
