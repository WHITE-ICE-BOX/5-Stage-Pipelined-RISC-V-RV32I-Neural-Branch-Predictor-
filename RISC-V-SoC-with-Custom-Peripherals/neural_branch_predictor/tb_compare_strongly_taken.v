...
// Strongly-taken loop test: addi x1=16, then countdown loop (bne x1,x0,-12).
// Branch taken 15 times, not-taken 1 time.
// Expected at end: x1=0, x30=99.
// Baseline (always-NT) accuracy = 1/16 = 6.25%.
// Neural BP should converge to "strongly taken" within a few iterations.

module tb_compare_strongly_taken;
    ...
    initial begin
        ...
        $display("\n==============================================================================");
        $display(" Strongly-Taken Loop: Baseline CPU vs Innovation CPU");
        $display(" Program: countdown x1=16->0, bne x1,x0,-12");
        $display(" Expected: 15 taken + 1 not-taken, x1=0, x30=99");
        $display("==============================================================================");
    ...
    initial begin
        $readmemb("/home/chiawei/SiliconAwards/RISC-V-SoC-with-Custom-Peripherals/neural_branch_predictor/instr_data_strongly_taken.txt",
    ...
    // writeback monitor: track x1, x30 in innovation CPU
    always @(posedge clk) begin
    ...
    initial begin
        ...
        $display("------------------------------------------------------------------------------");
        $display("STRONGLY-TAKEN LOOP -- FINAL COMPARISON");
        $display("------------------------------------------------------------------------------");
        ...
        $display("  x30 final = %0d (expected 99)", bp_x30_final);
        if ((bp_x1_final == 32'd0) && (bp_x30_final == 32'd99))
            $display("  STATUS: PASS");
        else
            $display("  STATUS: FAIL");
        $display("------------------------------------------------------------------------------");
