...
// Alternating-forward branch test.
// Loop: x1 counts from 16 down to 1.
//   andi x3, x1, 1          -- test odd/even
//   bne  x3, x0, +8         -- FORWARD branch: taken when x1 odd (skips addi x2)
//   addi x2, x2, 1          -- incremented only when x1 even
//   addi x1, x1, -1
//   bne  x1, x0, -24        -- BACK branch: always taken except last iteration
//
// Forward branch pattern: NT,T,NT,T,NT,T,NT,T,NT,T,NT,T,NT,T,NT,T (strict alternating)
// Back branch: 15 taken + 1 not-taken.
// Baseline on forward: 50% accuracy (always-NT gets NT correct, T wrong).
// Neural BP with GHR can detect the correlation and predict the alternating pattern.
// Expected at end: x1=0, x2=8, x30=66.

module tb_compare_alternating_forward;
    ...
    initial begin
        ...
        $display("\n==============================================================================");
        $display(" Alternating Forward Branch: Baseline CPU vs Innovation CPU");
        $display(" Program: x1=16 countdown; forward bne alternates NT,T,NT,T,...");
        $display(" Back branch: 15T+1NT. Expected: x1=0, x2=8, x30=66");
        $display("==============================================================================");
    ...
    initial begin
        $readmemb("/home/chiawei/SiliconAwards/RISC-V-SoC-with-Custom-Peripherals/neural_branch_predictor/instr_data_alternating_forward.txt",
    ...
    initial begin
        ...
        $display("------------------------------------------------------------------------------");
        $display("ALTERNATING FORWARD BRANCH -- FINAL COMPARISON");
        $display("------------------------------------------------------------------------------");
        ...
        $display("  x2  final = %0d (expected 8)",  bp_x2_final);
        $display("  x30 final = %0d (expected 66)", bp_x30_final);
        if ((bp_x1_final == 32'd0) && (bp_x2_final == 32'd8) && (bp_x30_final == 32'd66))
            $display("  STATUS: PASS");
        else
            $display("  STATUS: FAIL");
        $display("------------------------------------------------------------------------------");
