...
// Nested loop test: outer=5 iterations, inner=4 iterations each.
// Inner branch (bne x6,x0,-12): taken 3x per outer, not-taken 1x per outer -> 15T+5NT total.
// Outer branch (bne x5,x0,-40): taken 4x, not-taken 1x.
// Total branches: 25 inner + 5 outer = 25. (inner 20, outer 5)
// GHR correlation should help the neural predictor distinguish end-of-inner-loop from mid-loop.
// Expected at end: x5=0, x6=0, x30=55.

module tb_compare_nested_loop;
    ...
    reg [31:0] bp_x5_final;
    reg [31:0] bp_x6_final;
    ...
    initial begin
        ...
        $display("\n==============================================================================");
        $display(" Nested Loop: Baseline CPU vs Innovation CPU");
        $display(" Program: outer=5 x inner=4, inner bne x6,x0,-12, outer bne x5,x0,-40");
        $display(" Expected: x5=0, x6=0, x30=55");
        $display("==============================================================================");
    ...
    initial begin
        $readmemb("/home/chiawei/SiliconAwards/RISC-V-SoC-with-Custom-Peripherals/neural_branch_predictor/instr_data_nested_loop.txt",
    ...
    always @(posedge clk) begin
        if (!reset && bp_wb_re) begin
            case (bp_wb_rd)
                5'd5:  bp_x5_final  <= bp_wb_data;
                5'd6:  bp_x6_final  <= bp_wb_data;
    ...
    initial begin
        ...
        $display("------------------------------------------------------------------------------");
        $display("NESTED LOOP -- FINAL COMPARISON");
        $display("------------------------------------------------------------------------------");
        ...
        $display("  x5  final = %0d (expected 0)",  bp_x5_final);
        $display("  x6  final = %0d (expected 0)",  bp_x6_final);
        $display("  x30 final = %0d (expected 55)", bp_x30_final);
        if ((bp_x5_final == 32'd0) && (bp_x6_final == 32'd0) && (bp_x30_final == 32'd55))
            $display("  STATUS: PASS");
        else
            $display("  STATUS: FAIL");
        $display("------------------------------------------------------------------------------");
