...
// NT-biased workload test.
// Loop 16×:
//   beq x1, x2, +8   x1=99, x2=16..1: NEVER taken (always NT, 16 branches)
//   addi x3, x3, 1   always executes (beq never taken)
//   addi x2, x2, -1
//   bne  x2, x0, -16  back branch (15T + 1NT)
//
// Total: 32 branches (16 beq-NT + 15 bne-T + 1 bne-NT).
// Baseline always-NT: 17/32 = 53.13% accuracy, 15 mispredictions, 30 penalty cycles.
// Innovation: bimodal quickly saturates NT for beq (high confidence → no neural);
//             bimodal/neural learns strongly-taken for bne.
// Expected at end: x1=99, x3=16, x30=88.

module tb_compare_nt_biased;
    ...
    // writeback monitor (innovation)
    reg [31:0] bp_x3_final;
    reg [31:0] bp_x30_final;
    ...
    assign bp_wb_re   = u_bp.top_1.wb_re;
    assign bp_wb_rd   = u_bp.top_1.wb_rd;
    assign bp_wb_data = u_bp.top_1.wb_data;
    ...
    initial begin
        ...
        step_hist_0 = 0; step_hist_1 = 0; step_hist_2 = 0;
        step_hist_3 = 0; step_hist_4 = 0; step_hist_5 = 0;
        step_hist_6 = 0; step_hist_7 = 0; step_hist_8 = 0;
        ...
        $display("\n==============================================================================");
        $display(" NT-Biased Workload: Baseline CPU vs Innovation CPU");
        $display(" Program: beq (always NT, 16x) + bne back (15T+1NT)");
        $display(" Baseline: 17/32=53.13%%, 15 mispred, 30 penalty");
        $display(" Expected: x3=16, x30=88");
        $display("==============================================================================");
    ...
    initial begin
        $readmemb("/home/chiawei/SiliconAwards/RISC-V-SoC-with-Custom-Peripherals/neural_branch_predictor/instr_data_nt_biased.txt",
    ...
    always @(posedge clk) begin
        if (!reset && bp_cond_branch) begin
            ...
            if (bp_use_neural) begin
                ...
                if (bp_pred_taken == bp_actual_taken) bp_neural_correct = bp_neural_correct + 1;
                else                                  bp_neural_wrong   = bp_neural_wrong   + 1;
            end else begin
                bp_bimodal_path_count = bp_bimodal_path_count + 1;
                if (bp_pred_taken == bp_actual_taken) bp_bimodal_path_correct = bp_bimodal_path_correct + 1;
                else                                  bp_bimodal_path_wrong   = bp_bimodal_path_wrong   + 1;
            end
            bp_total_used_steps = bp_total_used_steps + bp_used_steps;
            if (bp_used_steps == 8) bp_full_step_count  = bp_full_step_count  + 1;
            else                    bp_early_exit_count = bp_early_exit_count + 1;
            case (bp_used_steps)
                0: step_hist_0 = step_hist_0+1; 1: step_hist_1 = step_hist_1+1;
                2: step_hist_2 = step_hist_2+1; 3: step_hist_3 = step_hist_3+1;
                4: step_hist_4 = step_hist_4+1; 5: step_hist_5 = step_hist_5+1;
                6: step_hist_6 = step_hist_6+1; 7: step_hist_7 = step_hist_7+1;
                8: step_hist_8 = step_hist_8+1; default: begin end
            endcase
            $display("[BP  ] t=%0d pc=%0d pred=%0d actual=%0d neural=%0d steps=%0d total=%0d correct=%0d mis=%0d pen=%0d",
                     $time, bp_pc, bp_pred_taken, bp_actual_taken,
                     bp_use_neural, bp_used_steps,
    ...
    always @(posedge clk) begin
        if (!reset && bp_wb_re) begin
            case (bp_wb_rd)
                5'd3:  bp_x3_final  <= bp_wb_data;
                5'd30: bp_x30_final <= bp_wb_data;
                default: begin end
            endcase
        end
    end
    ...
    initial begin
        ...
        $display("------------------------------------------------------------------------------");
        $display("NT-BIASED -- FINAL COMPARISON");
        $display("------------------------------------------------------------------------------");
        ...
        $display("Innovation Neural Ratio (x10000): %0d", bp_neural_ratio_x10000);
        $display("Innovation Avg Steps  (x10000)  : %0d", bp_avg_steps_x10000);
        $display("------------------------------------------------------------------------------");
        ...
        $display("  step 0:%0d 1:%0d 2:%0d 3:%0d 4:%0d 5:%0d 6:%0d 7:%0d 8:%0d",
                 step_hist_0,step_hist_1,step_hist_2,step_hist_3,
                 step_hist_4,step_hist_5,step_hist_6,step_hist_7,step_hist_8);
        $display("------------------------------------------------------------------------------");
        ...
        $display("------------------------------------------------------------------------------");
        $display("Functional Check (Innovation CPU):");
        $display("  x3  final = %0d (expected 16)", bp_x3_final);
        $display("  x30 final = %0d (expected 88)", bp_x30_final);
        if ((bp_x3_final == 32'd16) && (bp_x30_final == 32'd88))
            $display("  STATUS: PASS");
        else
            $display("  STATUS: FAIL");
        $display("------------------------------------------------------------------------------");
